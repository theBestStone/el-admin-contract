        ...
        <div class=" bg-white  tempbox" style="height:100%">
            ...
            <div class=" flex h100 sign-body2">
                ...
                <div class="rightbox bg-white"  :class="rightShowTag?'':'leftboxHide'">
                    ...
                    <div class="margin-top-sm  padding-lr" >
                        <!-- v-if="activeFile.areaTag" -->
                    <div class="sign-way-title">待填写内容 
                     
                    </div>
                    <div>
                       <div v-for="(item,index) in rectsPrint" v-if="(item.chapterType==ChapterTypes.TEXTAREA||item.chapterType==ChapterTypes.TEXT||item.chapterType==ChapterTypes.TEXTELEMENTAREA||item.chapterType==ChapterTypes.TEXTELEMENT)&&item.code==-99" class="margin-top">
...
import {downloadFile2,downloadFileTemp} from '../../../api';
import {ChapterTypes,calNum,uuid,calNumArea,verifyElementVal,elementSettingFake} from '../contract/signUtile.js'
    ...
    methods: {
        ...
        getId(tag) {
            ...
            this.contId =this.$route.query.contId; // this.$store.getters.contId;
             this.myname =this.$route.query.myname; // this.$store.getters.contId;
        },
        getType(){
        ...
        async getById(){
          ...
          let {data} = await findTemplate({templateNo:this.tempId});
      ...
      async tempAdd(type,code){
            let {errorObj,arrData} = verifyElementVal({
            ...
            model.files =arrFile;
            model.initatorSetting = JSON.stringify(arrData);
