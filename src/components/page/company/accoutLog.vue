  ...
  <div class="padding">
      ...
      <el-card class=" bg-white margin-top">
        ...
        <el-table ref="table" v-loading="loading" :data="list" style="width: 100%;" @selection-change="selectionChangeHandler">
          <el-table-column  prop="operateModule" label="操作" />
          <el-table-column  prop="operateUsername" label="成员">
            <template slot-scope="scope">
              <!-- <p>{{scope.row.useName}}</p> -->
              <p>{{scope.row.operateUsername}}</p>
            </template>
          </el-table-column>
          <el-table-column  prop="createTime" :formatter="$util.YYMM"  label="日期" />
          <el-table-column  prop="operateIp" label="登录IP" />
...
import {operateLogList,exportOperateLog} from '../../../api/log'
  ...
  data() {
    ...
    methods: {
      async doSearch(value) {
        ...
        let {data}  = await operateLogList(formData);
        this.list = data.list
