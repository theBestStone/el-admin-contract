		...
		<el-card>
			<div class="searchBox">
				<div>
					...
					<el-button class="btn-check" @click="exportOpen" type="primary">导出
					</el-button>
				</div>
			</div>
			...
			<div style="margin-top: 20px;min-height:50vh">
				<el-table class="com-table" ref="multipleTable" :data="tableData" tooltip-effect="dark" size="small"
					...
					<el-table-column prop="isCharged" label="是否收费">
						<template slot-scope="scope">
							{{scope.row.isCharged==0?'否':'是'}}
						</template>
					</el-table-column>
...
<script>
	import {
		openApiLogList,exportOpenApiLog
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				openApiLogList(this.params).then(res => {
